...
export default function TypesList(props) {
  ...
  const getList = (category, namespace, inFilter, page, size, inSortCol, inSortOrd, componentId) => {
    ...
    let req = { 
      method: 'get', 
      url: `${(localStorage.getItem('lib-appbaseurl') ?? process.env.REACT_APP_BASE_URL)}/cms/type-definitions?include-total=true&page=${page}&items-per-page=${size}${inSortCol?`&sortby=${inSortCol} ${inSortOrd}`:``}${inFilter?`&filter=${inFilter}`:``}${category?`&category=${category}`:``}${namespace?`&namespace=${namespace}`:``}`, 
  ...
  const handleCloseNewType = (status) => {
    if (status) {
      handleRefreshList();
    }
    setNewOpen(false);
  ...
  const handleSetFilter = (inFilterStr, inCategory, inNamespace) => {
    setFilterStr(inFilterStr);
    setSelCategory(inCategory);
  ...
  useEffect(
    () => {
      ...
      if (updatedList) {
        ...
        getList(selCategory, selNamespace, filterStr, curPage, rowsPerPage, sortCol, sortOrd, 'contentsList');
      }
    },[updatedList]
    );
  ...
  return (
      <React.Fragment>
            ...
            {showFilter && <CustomTypeFilter onFilterChange={handleSetFilter} onFilterClose={handleResetFilter} namespaces={namespaces}/>}
            <Box height={showFilter?"65vh":"70vh"} 
              ...
              sx={{
                      ...
                      {<TableBody>
                        {rows.map((row) => (
                          <TableRow key={row.id} hover selected={selectedRow.id==row.id}>
                            ...
                            <TableCell align="left" onClick={() => {handleSelectRow(row, false)}}>{row.category}</TableCell>
                            <TableCell align="right" onClick={() => {handleSelectRow(row, false)}}>{row.version}</TableCell>
                            ...
                            <TableCell align="left" onClick={() => {handleSelectRow(row, true)}}>
                              <Stack direction="row" spacing={0}>
                                ...
                                {row.namespace!='System' && <IconButton size="small" variant="outlined" color="error" title="Delete object" onClick={() => { setDeleteOpen(true)  }}>
                                  <DeleteForeverIcon />
                                </IconButton>}
                ...
                <TypeNew
                  ...
                  newOpen = {newOpen} 
                  onCreateSuccess = {handleCloseNewType} 
                  token = {token} 
                ...
                >
                  ...
                  <DialogContent>
                    <HistoricDetails runRequest = {runRequest} token = {token} showBorder={showBorder} instanceURL = {`cms/type-definitions/${selectedRow.system_name}`}/>
                  </DialogContent>
                  <DialogActions>
