...
export default function DeleteACL(props) {
  ...
  const handleDelete = () => {
    ...
    let req = { 
      method: 'delete',
      url: `${(localStorage.getItem('lib-appbaseurl') ?? process.env.REACT_APP_BASE_URL)}/cms/permissions/${inObj.id}`, 
  ...
  return (
      ...
      <Dialog open={deleteOpen} onClose={handleClose}>
        <DialogTitle>Delete ACL {inObj.name}</DialogTitle>
        <DialogContent>
          <DialogContentText>
            Are you sure you want to delete the ACL: {inObj.description}?
          </DialogContentText>
        </DialogContent>
        <DialogActions>
