...
export default function DocumentVersionNew(props) {
  ...
  const [sameVersion, setSameVersion] = React.useState(false);
  ...
  const selectFile = (event) => {
    ...
    setFileName(inObj.name ?? curSelectedFile.name);
	}
  ...
  const handleCreate = () => {
    ...
    let data = {
      ...
      parent_folder_id: inObj.parent_folder_id,
      renditions:renditions
    };
    
    if (inObj.type!='cms_file') {
      data.properties = inObj.properties;
    }

    let req = { 
      method: 'post', 
      data: data,
      url: `${(localStorage.getItem('lib-appbaseurl') ?? process.env.REACT_APP_BASE_URL)}/cms/instances/file/${inObj.type}/${inObj.id}/${sameVersion==true?'currentVersion':'nextVersion'}`, 
  ...
  return (
      ...
      <Dialog open={newFileOpen} onClose={handleClose} maxWidth={'xl'} fullWidth>
        ...
        <div>
            ...
            />
            <FormGroup>
              <FormControlLabel control={<Switch checked={sameVersion} onChange={e => {setSameVersion(e.target.checked)}} name="sameVersion" />} label="Upload as same version" />
            </FormGroup>
          </div>
